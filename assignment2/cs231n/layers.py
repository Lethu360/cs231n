...
def affine_forward(x, w, b):
    ...
    dim_num = x[0].shape
    X = x.reshape(x.shape[0], np.prod(dim_num))
    out = X.dot(w) + b
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    layernorm = bn_param.get('layernorm', 0)  # I added
...
def batchnorm_backward(dout, cache):
    ...
    dzdx = 1 / cache['std']
    dzdu = -1 / cache['std']
    dzdv = -0.5 * (cache['x']-cache['mean']) * (cache['var']**-1.5)
    ...
    for idx in range(N):
      ...
      for i in range(0, H_pad-HH+1, stride):
        for j in range(0, W_pad-WW+1, stride):
          ...
          neuron += 1
      out[idx] = (w_row.dot(x_col) + b.reshape(F,1)).reshape(F, outH, outW)
...
def conv_backward_naive(dout, cache):
    ...
    for idx in range(N):
      out = dout[idx].reshape(F, outH*outW)
      w_out = (w_row.T).dot(out)
      
      dx_each = np.zeros((C, H_pad, W_pad))
      neuron = 0
      for i in range(0, H_pad-HH+1, stride):
        for j in range(0, W_pad-WW+1, stride):
          dx_each[:, i:i+HH, j:j+WW] += w_out[:, neuron].reshape(C, HH, WW)
          x_col[:, neuron] = x_pad[idx, :, i:i+HH, j:j+WW].reshape(C*HH*WW)
          neuron += 1
      dx[idx] = dx_each[:, pad:-pad, pad:-pad]
      dw += out.dot(x_col.T).reshape(F, C, HH, WW)
      db += out.sum(axis=1)
    ...
    for idx in range(N):
      ...
      for i in range(0, H-PH+1, stride):
        for j in range(0, W-PW+1, stride):
          ...
          neuron += 1
      out[idx] = out_col.reshape(C, outH, outW)
...
def max_pool_backward_naive(dout, cache):
    ...
    N, C, H, W = x.shape
    outH, outW = dout.shape[2], dout.shape[3]

    dx = np.zeros(x.shape)

    for idx in range(N):
      neuron = 0
      dout_row = dout[idx].reshape(C, outH*outW)
      for i in range(0, H-PH+1, stride):
        for j in range(0, W-PW+1, stride):
